    ...
    <r:script>
        ...
        $(document).ready(function () {
            ...
            var editor = CKEDITOR.replace('content',
            {
                ...
                filebrowserBrowseUrl: "${createLink(controller: 'crmContent', action: 'browse')}?status=${crmResourceRef.statusText}&reference=crmResourceRef@${crmResourceRef.ident()}",
                filebrowserUploadUrl: "${createLink(controller: 'crmContent', action: 'upload')}",
                filebrowserImageBrowseUrl: "${createLink(controller: 'crmContent', action: 'browse')}?pattern=image&status=${crmResourceRef.statusText}&reference=crmResourceRef@${crmResourceRef.ident()}",
...
<g:set var="textEditMode" value="${metadata.contentType.startsWith('text')}"/>
...
<div class="tabbable">
    <ul class="nav nav-tabs">
        <li class="active"><a href="#main" data-toggle="tab">
            <g:message code="crmContent.tab.edit.label" default="Content"/>
        </a></li>
        <li><a href="#files" data-toggle="tab">
            <g:message code="crmContent.tab.files.label" default="Files"/>
    ...
    <div class="tab-content">
        ...
        <g:uploadForm>
                ...
                <div class="row-fluid">
                    <f:with bean="crmResourceRef">
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    <input type="hidden" id="inputTags" name="tags" value="${crmResourceRef.getTagValue()?.join(',')}" class="input-xlarge">
                                </div>
                            </div>
            ...
            <div class="tab-pane" id="files">
                <tmpl:embedded view="${[id: 'files']}" bean="${crmResourceRef}" list="${children}" totalCount="${children.size()}"
                                    reference="${childReference}" openAction="show" referer="${request.forwardURI}"/>
            </div>
        </div>
    </div>
